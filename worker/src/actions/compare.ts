...
export async function compareDomains(body: CompareRequest, env: Env): Promise<Response> {
  if (!body.domain1 || !body.domain2) {
    return new Response(
      JSON.stringify({ error: "domain1 and domain2 are required" }),
  ...
  if (!d1 || !d1.includes(".") || !d2 || !d2.includes(".")) {
    return new Response(
  ...
  if (d1 === d2) {
    return new Response(
      JSON.stringify({ error: "Cannot compare a domain with itself" }),
      { status: 400, headers: { "Content-Type": "application/json", ...CORS_HEADERS } },
    );
  }
  ...
  const [resp1, resp2] = await Promise.all([
    analyzeDomain(d1, env),
    analyzeDomain(d2, env),
  ]);
  ...
  const score1 = data1.domain_score as { composite: number; grade: string; axes: Record<string, { score: number; weight: number }>; archetype: { detected: string; confidence: number } } | null;
  const score2 = data2.domain_score as { composite: number; grade: string; axes: Record<string, { score: number; weight: number }>; archetype: { detected: string; confidence: number } } | null;
