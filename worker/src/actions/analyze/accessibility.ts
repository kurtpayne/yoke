...
const GENERIC_LINK_TEXT =
  /^(click here|here|read more|learn more|more|link|this|go|details|info|see more|view more|continue|continue reading)$/i;
...
export function analyzeAccessibility(html: string): AccessibilityResult {
  ...
  if (htmlTagMatch) {
    ...
    if (langMatch?.[1]) {
      if (isValidLangCode(langMatch[1])) {
        checks.push({
          ...
          status: "pass",
          detail: `<html lang="${langMatch[1]}"> is set and valid`,
          impact: "serious",
        });
      } else {
        checks.push({
          name: "Language attribute",
          status: "warn",
          detail: `<html lang="${langMatch[1]}"> — unrecognized language code`,
          impact: "serious",
        });
      }
    } else {
      checks.push({
        name: "Language attribute",
        status: "fail",
        detail: "No lang attribute on <html> element. Screen readers cannot determine page language.",
        impact: "serious",
      });
    }
  } else {
    checks.push({
      name: "Language attribute",
      status: "fail",
      detail: "No <html> tag found — page may not be valid HTML",
      impact: "serious",
    });
  }
  ...
  if (hasViewport) {
    ...
    if (/user-scalable\s*=\s*no/i.test(viewportContent) || /maximum-scale\s*=\s*1([^0-9]|$)/i.test(viewportContent)) {
      checks.push({
        name: "Viewport meta",
        status: "warn",
        detail: "Viewport disables or limits user zoom — impacts users who need to enlarge content",
        impact: "serious",
      });
    } else {
      checks.push({
        name: "Viewport meta",
        status: "pass",
        detail: "Viewport meta tag present and allows user scaling",
        impact: "moderate",
      });
    }
  } else {
    checks.push({
      name: "Viewport meta",
      status: "fail",
      detail: "No viewport meta tag — page may not render properly on mobile devices",
      impact: "moderate",
    });
  }
  ...
  if (headingLevels.length === 0) {
    checks.push({
      ...
      status: "fail",
      detail: "No headings found. Headings help screen reader users navigate page structure.",
      impact: "serious",
    });
  } else {
    ...
    if (!hasH1) {
      checks.push({
        name: "Heading hierarchy",
        status: "fail",
        detail: `No <h1> found. ${headingLevels.length} headings present (h${sortedUnique.join(", h")}).`,
        impact: "serious",
      });
    } else if (skipped) {
      checks.push({
        name: "Heading hierarchy",
        status: "warn",
        detail: `Heading levels skip: h${sortedUnique.join(" → h")}. Skipping levels confuses screen reader navigation.`,
        impact: "moderate",
      });
    } else if (h1Count > 1) {
      checks.push({
        name: "Heading hierarchy",
        status: "warn",
        detail: `Multiple <h1> tags found (${h1Count}). Best practice is a single h1 per page.`,
        impact: "minor",
      });
    } else {
      checks.push({
        name: "Heading hierarchy",
        status: "pass",
        detail: `Valid hierarchy: h${sortedUnique.join(" → h")} (${headingLevels.length} headings total)`,
        impact: "serious",
      });
    }
  }
  ...
  if (imgTags.length > 0) {
    ...
    if (missingAlt === 0) {
      checks.push({
        name: "Image alt text",
        status: "pass",
        detail: `All ${imgTags.length} images have alt attributes (${withAlt} descriptive, ${decorative} decorative)`,
        impact: "critical",
      });
    } else if (pct >= 80) {
      checks.push({
        name: "Image alt text",
        status: "warn",
        detail: `${missingAlt} of ${imgTags.length} images missing alt text (${pct}% have it)`,
        impact: "critical",
      });
    } else {
      checks.push({
        name: "Image alt text",
        status: "fail",
        detail: `${missingAlt} of ${imgTags.length} images missing alt text (only ${pct}% have it)`,
        impact: "critical",
      });
    }
  } else {
  ...
  if (totalFormElements > 0) {
      ...
      if (
        ...
        if (idMatch?.[1]) {
          const labelForRegex = new RegExp(
            `<label[^>]*for\\s*=\\s*["']${idMatch[1].replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`,
            "i",
          );
    ...
    if (labeled === totalFormElements) {
      checks.push({
        name: "Form labels",
        status: "pass",
        detail: `All ${totalFormElements} form elements have associated labels`,
        impact: "critical",
      });
    } else if (pct >= 70) {
      checks.push({
        name: "Form labels",
        status: "warn",
        detail: `${totalFormElements - labeled} of ${totalFormElements} form elements lack labels (${pct}% labeled)`,
        impact: "critical",
      });
    } else {
      checks.push({
        name: "Form labels",
        status: "fail",
        detail: `${totalFormElements - labeled} of ${totalFormElements} form elements lack labels (only ${pct}% labeled)`,
        impact: "critical",
      });
    }
  }
  ...
  if (linkMatches.length > 0) {
    ...
    if (genericCount === 0) {
      checks.push({
        ...
        status: "pass",
        detail: `${linkMatches.length} links checked — all have descriptive text`,
        impact: "moderate",
      });
      ...
      checks.push({
        name: "Link text quality",
        status: "warn",
        detail: `${genericCount} link(s) with generic text ("${genericExamples.join('", "')}") — unhelpful for screen readers`,
        impact: "moderate",
      });
    } else {
      checks.push({
        name: "Link text quality",
        status: "fail",
        detail: `${genericCount} links with generic text like "${genericExamples.join('", "')}". Screen readers read these out of context.`,
        impact: "moderate",
      });
    }
  }
  ...
  const landmarkNames = [hasMain && "main", hasNav && "nav", hasHeader && "header", hasFooter && "footer"].filter(
    Boolean,
  );
  ...
  if (landmarkCount >= 3) {
    checks.push({
      name: "ARIA landmarks",
      status: "pass",
      detail: `${landmarkCount} landmarks found: ${landmarkNames.join(", ")}`,
      impact: "moderate",
    });
    ...
    checks.push({
      name: "ARIA landmarks",
      status: "warn",
      detail: `Only ${landmarkCount} landmark(s): ${landmarkNames.join(", ")}. Missing: ${missing.join(", ")}`,
      impact: "moderate",
    });
  } else {
    checks.push({
      name: "ARIA landmarks",
      status: "fail",
      detail: "No ARIA landmarks found (<main>, <nav>, <header>, <footer>, or role= equivalents)",
      impact: "moderate",
    });
  }
  ...
  if (hasSkipNav) {
    checks.push({
      ...
      status: "pass",
      detail: "Skip navigation link found near top of page",
      impact: "moderate",
    });
  } else {
    checks.push({
      name: "Skip navigation",
      status: "warn",
      detail: "No skip navigation link found. Keyboard users must tab through all navigation links.",
      impact: "moderate",
    });
  }
  ...
  checks.push({
    name: "Color contrast",
    status: "warn",
    detail: "Not checked — requires visual rendering. Test with browser DevTools or axe.",
    impact: "serious",
  });
