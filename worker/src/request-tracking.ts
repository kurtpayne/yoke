...
import { type Env, backgroundWork } from "./helpers";
...
export function detectClientType(request: Request): string {
  ...
  if (/^curl\b/.test(ua) || /^wget\b/.test(ua) || /^httpie\b/.test(ua) || ua === "" || /^python-/.test(ua) || /^go-http/.test(ua) || /^node-fetch/.test(ua)) return "cli";
  // If no origin and no referer, likely direct API call
  if (!origin && !request.headers.get("referer") && request.headers.get("accept")?.includes("application/json")) return "api";
  return "web";
}
    ...
    try {
      ...
      await env.STATS_DB.prepare(
        `INSERT INTO request_meta (ts, day, hour, endpoint, domain, client_type, country, status_code, latency_ms, visitor_hash)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(now, day, hour, meta.endpoint, meta.domain || null, clientType, country, meta.status, meta.latencyMs, visitorHash).run();
    } catch {
...
export async function getRequestAnalytics(db: D1Database, days: number): Promise<RequestAnalytics> {
  ...
  try {
    ...
    result.top_domains = ((topDomains.results || []) as { domain: string; cnt: number; avg_lat: number; uniq_visitors: number }[]).map(r => ({
      domain: r.domain, scans: r.cnt, avg_latency: Math.round(r.avg_lat || 0), unique_scanners: r.uniq_visitors,
    }));

    // Visitors per day
    const vpd = await db.prepare(
    ...
    result.by_country = ((countries.results || []) as { country: string; cnt: number; visitors: number }[]).map(r => ({
      country: r.country, requests: r.cnt, visitors: r.visitors,
    }));

    // Hourly distribution (UTC)
    const hourly = await db.prepare(
      `SELECT hour, COUNT(*) as cnt FROM request_meta WHERE day >= ? GROUP BY hour ORDER BY hour`
    ).bind(cutoff).all();
    ...
    ).bind(cutoff).first<{ multi: number }>();
