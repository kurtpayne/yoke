import { AlertTriangle, Check, ChevronDown, ChevronRight, Code, XCircle } from "lucide-react";
import { useState } from "react";
import type { AnalysisResult, FieldValidation, SchemaValidationItem } from "../utils/types";
...
function validationStatusBadge(status: SchemaValidationItem["status"]) {
  switch (status) {
    case "complete":
      return <StatusBadge status="pass" label="Valid" />;
    case "partial":
      return <StatusBadge status="warn" label="Partial" />;
    case "missing_required":
      return <StatusBadge status="fail" label="Missing Required" />;
  }
}
...
function SchemaTypeCard({ validation }: { validation: SchemaValidationItem }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--border-muted)" }}>
      ...
      >
        {expanded ? (
          <ChevronDown size={12} style={{ color: "var(--dim)" }} />
        ) : (
          <ChevronRight size={12} style={{ color: "var(--dim)" }} />
        )}
        <span style={{ fontWeight: 600 }}>{validation.type}</span>
        {validation.name && (
          <span style={{ color: "var(--dim)", fontSize: "11px", fontWeight: 400 }}>— {validation.name}</span>
        )}
          ...
          {validation.required_fields.length > 0 && (
            <div className="mb-2">
              ...
              {validation.required_fields.map((f) => (
                <div key={f.field} className="flex items-center gap-2 py-1" style={{ fontSize: "11px" }}>
          ...
          {validation.recommended_fields.length > 0 && (
            <div>
              ...
              {validation.recommended_fields.map((f) => (
                <div key={f.field} className="flex items-center gap-2 py-0.5" style={{ fontSize: "11px" }}>
                  {fieldStatusIcon(f.status)}
                  <span
                    style={{
                      fontFamily: "var(--font-mono)",
...
export function StructuredDataPanel({ data }: { data: AnalysisResult }) {
  ...
  return (
    ...
    >
      ...
      >
        <span>
          {sd.total_items} schema type{sd.total_items !== 1 ? "s" : ""} detected
        </span>
