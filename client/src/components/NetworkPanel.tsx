...
import { blocklistCliCommands, CliButton, httpProtocolCliCommands, ipInfoCliCommands } from "./CliModal";
...
export function IpInfoPanel({ data }: { data: AnalysisResult }) {
  ...
  return (
    <Panel
      title="IP & Hosting"
      icon={<MapPin size={14} />}
      badge={<CliButton commands={ipInfoCliCommands(ip.ip)} domain={data.domain} ip={ip.ip} />}
    >
      ...
      {ip.lat != null && ip.lon != null && (
        <DataRow label="Coordinates" value={`${ip.lat.toFixed(4)}, ${ip.lon.toFixed(4)}`} />
      )}
    ...
    <Panel
      ...
      badge={
        <div className="flex items-center gap-1.5">
          {data.ip_info?.ip && (
            <CliButton commands={blocklistCliCommands(data.ip_info.ip)} domain={data.domain} ip={data.ip_info.ip} />
          )}
          {listedCount > 0 ? (
            <StatusBadge status="fail" label={`${listedCount} LISTED`} />
          ) : (
            <StatusBadge status="pass" label="ALL CLEAN" />
          )}
        </div>
      }
    >
        ...
        <div key={`net-${i}`} className="data-row">
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{ background: bl.listed ? "var(--danger)" : "var(--success)" }}
            />
            <span
              style={{
                fontFamily: "var(--font-mono)",
                fontSize: "12px",
                color: bl.listed ? "var(--danger)" : "var(--text)",
              }}
            >
              {bl.name}
            </span>
  ...
  return (
    ...
    >
      <DataRow
        label="HTTP/2"
        value={
          <StatusBadge status={proto.http2 ? "pass" : "neutral"} label={proto.http2 ? "Supported" : "Not detected"} />
        }
      />
      <DataRow
        label="HTTP/3"
        value={
          <StatusBadge status={proto.http3 ? "pass" : "neutral"} label={proto.http3 ? "Supported" : "Not detected"} />
        }
      />
      {proto.alt_svc && (
        <DataRow
          label="Alt-Svc"
          value={
            <span style={{ fontSize: "10px" }} className="break-all">
              {proto.alt_svc.length > 80 ? `${proto.alt_svc.slice(0, 80)}…` : proto.alt_svc}
            </span>
          }
          copyValue={proto.alt_svc}
        />
      )}
    </Panel>
  );
