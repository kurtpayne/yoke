    ...
    <Panel
      ...
      badge={
        <div className="flex items-center gap-1.5">
          ...
          <Tooltip
            text={`${ct.subdomains.length} unique subdomains discovered from ${ct.total_certs} SSL certificate issuances in public CT logs`}
          >
        ...
        {ct.issuers.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-1">
              {ct.issuers.map((issuer, _i) => (
                <span key={issuer} className="badge badge-info" style={{ fontSize: "10px" }}>
                  {issuer}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="mt-1" style={{ maxHeight: "200px", overflowY: "auto" }}>
              <div className="flex flex-wrap gap-1.5">
                {ct.subdomains.slice(0, 50).map((sub, _i) => (
                  <span
                    key={sub}
                    style={{
                ...
                {ct.subdomains.length > 50 && (
                  <span
                    style={{ fontFamily: "var(--font-ui)", fontSize: "11px", color: "var(--dim)", padding: "2px 6px" }}
                  >
          ...
          <div>
            <div
              style={{
                ...
                fontSize: "13px",
                fontWeight: 600,
                color: gh.green ? "var(--success)" : "var(--dim)",
              }}
            >
            ...
            {gh.hosted_by && (
              <div style={{ fontFamily: "var(--font-ui)", fontSize: "11px", color: "var(--dim)", marginTop: "2px" }}>
                Hosted by:{" "}
                {gh.hosted_by_website ? (
                  <a
                    href={gh.hosted_by_website}
                    target="_blank"
                    rel="noopener noreferrer"
      ...
      <div className="p-3 space-y-2">
        {wk.endpoints.map((ep, i) => (
          <div
            ...
            style={{ borderBottom: i < wk.endpoints.length - 1 ? "1px solid var(--border)" : "none" }}
          >
            <span style={{ marginTop: "2px" }}>
              {ep.found ? (
                <CheckCircle2 size={12} style={{ color: "var(--success)" }} />
              ) : (
                <XCircle size={12} style={{ color: "var(--dim)", opacity: 0.4 }} />
              )}
            </span>
              ...
              <div className="flex items-center gap-2">
                <span
                  style={{
                    fontFamily: "var(--font-mono)",
                    fontSize: "11px",
                    color: ep.found ? "var(--fg)" : "var(--dim)",
                  }}
                >
                  {ep.path}
                </span>
                <span style={{ fontFamily: "var(--font-ui)", fontSize: "10px", color: "var(--dim)" }}>{ep.name}</span>
              </div>
              {ep.found && ep.data && (
                <div style={{ marginTop: "4px" }}>
                      ...
                      {Array.isArray(ep.data.top_partners) && (ep.data.top_partners as string[]).length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {(ep.data.top_partners as string[]).slice(0, 6).map((p, j) => (
                            <span key={j} className="badge badge-info" style={{ fontSize: "9px" }}>
                              {p}
                            </span>
                          ))}
                        </div>
                  ...
                  {ep.name === "Web App Manifest" && (
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {ep.data.display ? (
                        <span className="badge badge-info" style={{ fontSize: "9px" }}>
                          display: {String(ep.data.display)}
                        </span>
                      ) : null}
                      {ep.data.theme_color ? (
                        <span className="badge badge-info" style={{ fontSize: "9px" }}>
                          <span
                            style={{
                              ...
                              height: "8px",
                              borderRadius: "50%",
                              background: String(ep.data.theme_color),
                              marginRight: "4px",
                      ...
                      {(ep.data.icon_count as number) > 0 && (
                        <span className="badge badge-info" style={{ fontSize: "9px" }}>
                          {ep.data.icon_count as number} icons
                        </span>
                      )}
                    </div>
                  )}
  ...
  return (
    <Panel
      ...
      badge={
        ...
        >
          <span style={{ cursor: "help" }}>
            <StatusBadge
              status={caa.has_caa ? "pass" : "warn"}
              label={
                caa.has_caa ? `${caa.records.length} CA${caa.records.length !== 1 ? "s" : ""} authorized` : "No CAA"
              }
            />
          </span>
        </Tooltip>
    ...
    >
      <div className="p-3 space-y-2">
        {caa.has_caa ? (
          <>
            {caa.records.map((rec, _i) => (
              <DataRow
                key={`caa-${rec.value}`}
                label={
                  <div className="flex items-center gap-2">
                    <span
                      style={{ fontFamily: "var(--font-ui)", fontSize: "12px", fontWeight: 600, color: "var(--fg)" }}
                    >
                      {rec.ca_name}
                    </span>
                    <span className="badge badge-info" style={{ fontSize: "9px" }}>
...
export function GreynoisePanel({ data }: { data: AnalysisResult }) {
  ...
  const classLabel =
    gn.classification === "malicious" ? "Malicious" : gn.classification === "benign" ? "Benign" : "Unknown";
      ...
      badge={
        <div className="flex items-center gap-1.5">
          ...
          <Tooltip text={`GreyNoise classification: ${classLabel} — based on observed internet scanning behavior`}>
            <span style={{ cursor: "help" }}>
              <StatusBadge
                status={gn.classification === "malicious" ? "fail" : gn.classification === "benign" ? "pass" : "info"}
                label={classLabel}
              />
            </span>
          </Tooltip>
    ...
    >
      <div className="p-3 space-y-2">
        ...
        <DataRow
          ...
          value={<span style={{ color: classColor, fontWeight: 600 }}>{classLabel}</span>}
