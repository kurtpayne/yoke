import { AlertCircle, CheckCircle, Image, Share2 } from "lucide-react";
    ...
    >
      ...
      <div className="px-4 py-3">
        <div
          ...
          style={{ border: "1px solid var(--border-muted)", maxWidth: "400px" }}
        >
          {img ? (
      ...
      <DataRow
        label="og:type"
        value={og.type || <StatusBadge status="warn" label="Missing" />}
        copyValue={og.type || undefined}
      />
      <DataRow
        label="og:url"
        value={
          og.url ? (
            <span style={{ fontSize: "10px", wordBreak: "break-all" }}>{og.url}</span>
          ) : (
            <StatusBadge status="warn" label="Missing" />
          )
        }
        copyValue={og.url || undefined}
      />
      <DataRow
        label="og:locale"
        value={og.locale || <StatusBadge status="neutral" label="Not set" />}
        copyValue={og.locale || undefined}
      />
      <DataRow
        label="og:site_name"
        value={og.site_name || <StatusBadge status="warn" label="Missing" />}
        copyValue={og.site_name || undefined}
      />
      <DataRow
        label="twitter:card"
        value={twitter.card || <StatusBadge status="warn" label="Missing" />}
        copyValue={twitter.card || undefined}
      />
      <DataRow
        label="twitter:site"
        value={twitter.site || <StatusBadge status="neutral" label="Not set" />}
        copyValue={twitter.site || undefined}
      />
      <DataRow
        label="twitter:creator"
        value={twitter.creator || <StatusBadge status="neutral" label="Not set" />}
        copyValue={twitter.creator || undefined}
      />
      ...
      {sm.missing.length > 0 && (
        <>
          ...
          <div className="px-4 py-2 flex flex-wrap gap-1.5">
            {sm.missing.map((tag, _i) => (
              <span key={tag} className="badge badge-warn" style={{ fontSize: "11px" }}>
                <AlertCircle size={9} style={{ marginRight: "3px" }} />
                {tag}
              </span>
            ))}
          </div>
      ...
      {sm.missing.length === 0 && (
        <div className="px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span style={{ fontFamily: "var(--font-ui)", fontSize: "12px", color: "var(--success)" }}>
              All essential social meta tags present
            </span>
          </div>
        </div>
      )}
