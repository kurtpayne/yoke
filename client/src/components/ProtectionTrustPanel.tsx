import { AlertTriangle, Check, ExternalLink, Shield, ShieldAlert, ShieldCheck, X } from "lucide-react";
          ...
          waf.detected ? (
            <div className="flex items-center gap-1.5">
              ...
              <Tooltip text={`Confidence: ${waf.confidence}. Evidence: ${waf.signals.join(", ")}`}>
                <span
                  className={`badge badge-${waf.confidence === "high" ? "pass" : waf.confidence === "medium" ? "info" : "warn"}`}
...
function TrustSignalRow({
  ...
  signal: { name: string; present: boolean; value: string | null; severity: string; importance?: string };
}) {
    ...
    <DataRow
      label={
        <span
          ...
          style={{ fontFamily: "var(--font-ui)", fontSize: "11px", color: "var(--text)" }}
        >
          {signal.present ? (
    ...
    <Panel
      ...
      badge={
        <div className="flex items-center gap-1.5">
          ...
          <StatusBadge
            status={
              corePresent >= coreTotal && coreTotal > 0
                ? "pass"
                : corePresent >= Math.ceil(coreTotal / 2)
                  ? "info"
                  : "neutral"
            }
