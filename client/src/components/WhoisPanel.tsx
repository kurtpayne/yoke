...
export function WhoisPanel({ data }: { data: AnalysisResult }) {
  ...
  const expiryBadge = rdap.days_until_expiry != null
  ...
  return (
    <Panel title="WHOIS / RDAP" icon={<FileText size={14} />} badge={
      ...
      {rdap.domain_age_days != null && (
        <DataRow
          label="Domain Age"
          value={rdap.domain_age_days > 365
      ...
      {rdap.status.length > 0 && (
        <div className="px-4 py-2.5 flex flex-wrap gap-1.5" style={{ borderTop: "1px solid var(--border-muted)" }}>
          {rdap.status.map((s, i) => (
            <span key={s} className="badge badge-neutral" style={{ fontSize: "10px" }}>{s}</span>
          ))}
        </div>
      )}
    </Panel>
...
export function DomainExpiryPanel({ data }: { data: AnalysisResult }) {
  ...
  const color = days < 7 ? "var(--danger)" : days < 30 ? "var(--danger)" : days < 90 ? "var(--warning)" : "var(--success)";
  ...
  return (
    <Panel title="Domain Expiry" icon={icon}>
      <div className="flex items-center gap-4 p-4">
        <div className="flex flex-col items-center justify-center rounded-lg p-3" style={{ background: `color-mix(in srgb, ${color} 10%, transparent)`, minWidth: "80px" }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: "28px", fontWeight: 700, color, lineHeight: 1 }}>
            {days}
          </span>
          <span style={{ fontFamily: "var(--font-ui)", fontSize: "10px", color: "var(--dim)", marginTop: "4px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
            days left
          </span>
        </div>
        <div className="flex-1">
          <span className={`badge badge-${days < 30 ? "fail" : days < 90 ? "warn" : "pass"}`} style={{ marginBottom: "6px" }}>
            {label}
          </span>
          <p style={{ fontFamily: "var(--font-ui)", fontSize: "12px", color: "var(--dim)", marginTop: "6px", lineHeight: "18px" }}>
