  ...
  if (!tech || tech.length === 0)
    return (
      <Panel
        title="Tech Stack"
        icon={<Layers size={14} />}
        badge={<CliButton commands={techStackCliCommands(data.domain)} domain={data.domain} />}
      >
  ...
  return (
    ...
    >
      {[...grouped.entries()].map(([category, items]) => (
        <div key={category}>
          <div className="sub-section" style={{ color: "var(--dim)", fontSize: "10px" }}>
            {category}
          </div>
          {items.map((item, _i) => (
            <DataRow
      ...
      <div className="px-4 py-2.5 flex items-center gap-4 border-t" style={{ borderColor: "var(--border)" }}>
        ...
        {(["high", "medium", "low"] as const).map((level) => (
          <span
            ...
            style={{ fontSize: "9px", fontFamily: "var(--font-ui)", color: confidenceColor[level] }}
          >
