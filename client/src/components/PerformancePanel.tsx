import { BarChart3, ExternalLink, Gauge, Leaf, Monitor, Smartphone, Users } from "lucide-react";
      ...
      badge={
        <div className="flex items-center gap-1.5">
          ...
          {hasError && !hasCrux ? (
            <StatusBadge
              status="warn"
              label={perf!.error!.length > 40 ? `${perf!.error!.slice(0, 37)}…` : perf!.error!}
            />
          ) : undefined}
        </div>
      }
        ...
        <div style={{ borderBottom: "1px solid var(--border-muted)" }}>
          <div className="px-4 py-2" style={{ background: "color-mix(in srgb, var(--success) 5%, transparent)" }}>
            <div className="flex items-center gap-1.5">
              ...
              <span
                style={{ fontFamily: "var(--font-ui)", fontSize: "11px", fontWeight: 600, color: "var(--success)" }}
              >
              ...
              <DataRow
                label={
                  <span className="flex items-center gap-1">
                    LCP{" "}
                    <Tooltip
                      text="Largest Contentful Paint — p75 from real Chrome users over 28 days. Under 2.5s is good."
                      help
                    />
                  </span>
                }
                value={<VitalBadge label={formatMs(crux.lcp_p75)} value={crux.lcp_p75} good={2500} poor={4000} />}
              />
            )}
            {crux.fcp_p75 != null && (
              <DataRow
                label={
                  <span className="flex items-center gap-1">
                    FCP <Tooltip text="First Contentful Paint — p75 from real Chrome users. Under 1.8s is good." help />
                  </span>
                }
                value={<VitalBadge label={formatMs(crux.fcp_p75)} value={crux.fcp_p75} good={1800} poor={3000} />}
              />
            )}
            {crux.cls_p75 != null && (
              <DataRow
                label={
                  <span className="flex items-center gap-1">
                    CLS <Tooltip text="Cumulative Layout Shift — p75 from real Chrome users. Under 0.1 is good." help />
                  </span>
                }
                value={<VitalBadge label={crux.cls_p75.toFixed(3)} value={crux.cls_p75} good={0.1} poor={0.25} />}
              />
            )}
            {crux.inp_p75 != null && (
              <DataRow
                label={
                  <span className="flex items-center gap-1">
                    INP{" "}
                    <Tooltip
                      text="Interaction to Next Paint — a Core Web Vital measuring responsiveness. Under 200ms is good. Only available from real user data."
                      help
                    />
                  </span>
                }
                value={<VitalBadge label={formatMs(crux.inp_p75)} value={crux.inp_p75} good={200} poor={500} />}
              />
            )}
            {crux.ttfb_p75 != null && (
              <DataRow
                label={
                  <span className="flex items-center gap-1">
                    TTFB <Tooltip text="Time to First Byte — p75 from real Chrome users. Under 800ms is good." help />
                  </span>
                }
                value={<VitalBadge label={formatMs(crux.ttfb_p75)} value={crux.ttfb_p75} good={800} poor={1800} />}
              />
            )}
          </div>
          ...
          {crux.form_factors && (
            <div className="px-4 py-1.5" style={{ borderTop: "1px solid var(--border-muted)" }}>
              <span style={{ fontFamily: "var(--font-ui)", fontSize: "10px", color: "var(--dim)" }}>
                Device mix: 🖥 {Math.round(crux.form_factors.desktop * 100)}% · 📱{" "}
                {Math.round(crux.form_factors.phone * 100)}% · 📟 {Math.round(crux.form_factors.tablet * 100)}%
              </span>
            </div>
          )}
        </div>
...
export function CarbonPanel({ data }: { data: AnalysisResult }) {
  ...
  const color =
    co2 != null ? (co2 < 0.5 ? "var(--success)" : co2 < 1.0 ? "var(--warning)" : "var(--danger)") : "var(--dim)";
      ...
      <div className="flex items-center gap-4 p-4">
        {co2 != null && (
          <div
            ...
            style={{ background: `color-mix(in srgb, ${color} 8%, transparent)`, minWidth: "80px" }}
          >
            ...
            <div>
              ...
              <div className="w-full h-1.5 rounded-full" style={{ background: "var(--border)" }}>
                <div
                  ...
                  style={{ width: `${Math.round(cleaner * 100)}%`, background: color }}
                />
              </div>
            </div>
          )}
          <StatusBadge
            status={carbon.green ? "pass" : "neutral"}
            label={carbon.green ? "Green hosted" : "Standard hosting"}
          />
        </div>
      </div>
    </Panel>
