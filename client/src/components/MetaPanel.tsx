import { FileSearch, Info, Bot } from "lucide-react";
...
export function MetaPanel({ data }: { data: AnalysisResult }) {
  ...
  return (
    <Panel title="Site Meta" icon={<Info size={14} />}>
      ...
      <DataRow
        label="Sitemap"
        value={meta.sitemap_detected ? (
          <div className="flex items-center gap-2">
            <StatusBadge status="pass" label="FOUND" />
            {meta.sitemap_page_count != null && (
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "10px", color: "var(--dim)" }}>{meta.sitemap_page_count} entries</span>
            )}
          </div>
...
export function RobotsDeepPanel({ data }: { data: AnalysisResult }) {
  ...
  if (parsed.is_missing) return (
    <Panel title="robots.txt Analysis" icon={<FileSearch size={14} />}>
      <div className="p-4">
        <StatusBadge status="warn" label="robots.txt not found" />
        <p style={{ fontFamily: "var(--font-ui)", fontSize: "12px", color: "var(--dim)", marginTop: "8px", lineHeight: "18px" }}>
    ...
    <Panel
      ...
      badge={
        <div className="flex items-center gap-1.5">
          ...
          {parsed.is_restrictive ? <StatusBadge status="warn" label="RESTRICTIVE" /> : <StatusBadge status="pass" label="STANDARD" />}
        </div>
      }
    >
      ...
      {llms.content && (
        <div>
          ...
          <div className="p-4" style={{ maxHeight: "200px", overflowY: "auto" }}>
            <pre style={{ fontFamily: "var(--font-mono)", fontSize: "11px", lineHeight: "18px", color: "var(--text-secondary)", whiteSpace: "pre-wrap", wordBreak: "break-all", margin: 0 }}>
              {llms.content}
            </pre>
          </div>
        </div>
